...
submitBtn.addEventListener('click', function(event) {
  ...
  // Local storage to stores data and that will not be deleted when the browser is closed, 
  ...
  hScoreDiv.classList.remove('d-none');

  for (var i = 0; i < users.length; i++) {

    var hScoreList = document.querySelector('ul');

    var li = document.createElement('li');

    li.textContent = `${users[i].userName}: ${users[i].score}`;

    hScoreList.append(li);
  }
});
